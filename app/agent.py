...
def render(xgb_model, ae_model_artifacts, mlp_model, shap_explainer, metadata, dataset):
    ...
    with st.container(border=True):
        ...
        with col_btn:
            run_stress = st.button("Simulate thermal stress", type="primary", use_container_width=True, key="agent_stress")
        with col_btn2:
            run_det = st.button("Run Determinism Check", type="primary", use_container_width=True, key="agent_det")
        with col_info:
            st.markdown("""
